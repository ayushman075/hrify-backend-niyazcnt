...
export const getLeaveLimits = asyncHandler(async (req, res) => {
    ...
    // --- ADDED .populate() CALLS HERE ---
    const leaveLimits = await LeaveLimit.find(filterConditions)
      .populate("employeeId", "firstName lastName employeeId") // Get Employee Name and ID
      .populate("postId", "title")                             // Get Post Title
      .populate("leaveDetails.leaveType", "leaveType")         // Get Leave Type Name
...
export const manualUpdateLeaveBalance = asyncHandler(async (req, res) => {
    const { leaveLimitId } = req.params;
    const { leaveTypeId, newRemaining, newUsed } = req.body;

    if (!leaveTypeId || newRemaining === undefined || newUsed === undefined) {
        return res.status(400).json(new ApiResponse(400, {}, "Missing required fields", false));
    }

    const leaveLimit = await LeaveLimit.findById(leaveLimitId);
    if (!leaveLimit) {
        return res.status(404).json(new ApiResponse(404, {}, "Leave limit record not found", false));
    }

    const detailIndex = leaveLimit.leaveDetails.findIndex(d => d.leaveType.toString() === leaveTypeId);
    
    if (detailIndex === -1) {
        return res.status(404).json(new ApiResponse(404, {}, "Leave type not found in employee's limit", false));
    }

    leaveLimit.leaveDetails[detailIndex].remainingLeaves = Number(newRemaining);
    leaveLimit.leaveDetails[detailIndex].usedLeaves = Number(newUsed);

    await leaveLimit.save();

    // Cache Invalidation
    await removeCache(`${CACHE_KEY.PREFIX}${leaveLimit.employeeId}`);
    await removeCachePattern(`${CACHE_KEY.LIST_PREFIX}*`);

    return res.status(200).json(new ApiResponse(200, leaveLimit, "Leave balance updated successfully", true));
});